...
**内容**：
  ...
  - `_handle_files` 端点：路径安全检查（仅 ~/$MEDIA_DIR/ + ~/$DATA_DIR/）
...
**影响面**：cron 产出洁净 ✅、用户交付文件（$DATA_DIR/、kb/）仍自动标注 ✅。2026-07-03 升级后 `_check_content_compliance()` 中顶层 `skip_for_paths` 检查缺失，手动补回 3 行。
...
**发现者**：第二用户的 Hermes（deepseek-v4-pro），2026-06-20
**回滚**：恢复删除的 4 行即可
...
1. **时序问题**：Agent 写 MEDIA: 标签时文件还未落盘到 `$MEDIA_DIR/`，`validate_media_delivery_path` 用 `strict=True` 解析路径，文件不存在静默返回 None。
...
**问题**：Agent 在大量工具调用后偶发空回复（DeepSeek V4 已知问题），或回复中遗漏 MEDIA: 标签——文件已通过 send-media-safe.sh 拷入 $MEDIA_DIR/ 但从未投递。P023（weixin.py 修复）只解决 gateway 侧静默失败，不解决 Agent 侧漏写标签。
...
- **文件**: `~/.hermes/plugins/l0-safety-guard/__init__.py`, `plugin.yaml`
- **改动**:
  1. 新增 `post_tool_call` hook + `_post_tool_call` 函数（35行）——拦截 子代理，存 delegation_id → goal 映射，同步 fallback 直落盘
...
- **代码量**: +180/+25/-60 行
- **背景**: 5B source-graded meta 建立后，TTL 仍用 6 类关键词分类（无法区分"用户亲口说"vs"LLM推理"、无时间衰减加权）。5C 构建独立置信度引擎——不止用于 memory TTL，后续可接入画像门控、深思筛选、告警评级等场景。fact_type 三分类（constant/mutable/transient）直接对应 arxiv 2604.11364 四层持久化语义。方案经四家多轮独立审查（内部+多模型+火山），25+3 全自动测试通过，dry-run 145条全保留。
...
- **改动**:
  ...
  4. 完整清洗公开仓库：HOST_MAIN, $COLLABORATOR, 用户, GROUP, 某外企, 某科技企业 等
- **代码量**: +100/-90 行
...
## P057 — Rule 18 推送连续二次授权门禁（2026-07-13）

- **文件**: `~/.hermes/scripts/rule18_push_gate.sh`, `pre-push hook`（两个仓库）, SOUL.md Rule 18
- **改动**:
  1. rule18_push_gate.sh（60行）——init→step1→step2 状态机，step2 后 60s 内有效
  2. pre-push hook 新增 Rule 18 授权检查（隐私扫描之前）
  3. SOUL.md 新增 Rule 18 + L1 导航 #26 场景路由
  4. 未授权时 hook 直接阻断，git push 无法执行
- **背景**: 2026-07-13 8次违规推送（含 force push）。即使用户说"推"也不够——需两次独立确认。门禁生效后，即使 Agent 绕过 Rule 18 直接调 git push，pre-push hook 也会因缺少授权时间戳而阻断。
- **关联**: pre_push_privacy_check.sh（P056）, Rule 0（默认拒绝）
- **session**: 20260713

- **2026-08-05 归因层升级 v1.5（R1-R13）**: timed out 词形缺口 / untrusted 剥离 / 404 / blocked 优先 / payload 防护 / success:true 直接 unknown / traceback 收紧 / patch 参数错误族 / refusing to write / session_id 等 13 条规则；100 样本一致率 83% → 97.8%（真错误子集）
- **2026-08-05 提取层 E1-E3（error_extract.py v1.0）**: 截断 JSON 容错解析（E1）/ success:true 前置过滤 92.1% 噪音消除（E2）/ exit_code 传递（E3）；提取定义版本化 EXTRACT_DEF_VERSION 防条数漂移（根治「609 无法复现」问题）；100 样本真错误子集归因一致率 97.8%

- **2026-08-05 CI #5（T71）**: CI skill 步③ 方案模板加「验收条件」段（L1/L2/L3 判定层+判定方式+通过标准）+ 步④ 审计检查单。非产品源码补丁——skill 文档模板增强，记录备查。

- **2026-08-06 CI #6（T72）**: 新插件 intent-drift-guard（pre_llm_call）——意图漂移检测，注入确认提醒不阻断。config.yaml plugins.enabled 白名单加入。三轮审计（deleg_726cd11d 3🔴 + DS/Volc 两轮 6🔴）全吸收。备份 plugins_archive/intent-drift-guard + config.yaml.bak.20260805_2355。

- **2026-08-06 CI #7（T73）**: memory-supersedes 插件 _post_tool_call 2 处 upsert_entry 加 session_id（记忆证据链回查）。备份 backups/memory-supersedes.__init__.py.bak.20260806_0015 + memory_meta.json.bak.20260806_0015。

- **2026-08-06 静默通知误发修复（T76）**: 主动重启（hermes-restart-oneshot.sh stop 超时强制 kill -9）→ systemd OnFailure 误触发 → alert-on-crash 无条件写 crash flag → 恢复后误发「🚨 Gateway 已恢复」。修复：restart 脚本 stop 前写 planned-restart 标记；alert-on-crash 检查标记存在则静默退出（不写 crash flag，清理标记）。真崩溃告警保留。同类扫描：baohe 用 systemctl restart 干净重启无此问题；watchdog 合理。备份 backups/*.bak.20260806_0040。

- **2026-08-06 上午 OOM 修复（T77，四 cron 报警根因闭环）**: 三次 OOM kill（8/5 00:34 / 8/6 00:33 / 8/6 04:18）打断画像日更 ×2 + prep ×1 → 画像 53h 未更新 → 鲜度检查 error + 心跳哨兵报警 + 聚合 91%。根因：session_search 重度 LLM cron 内存暴涨（gateway 基线 rss 8.9GB + 3 分钟涨 12GB+）。四层修复：①crash_self_heal.py v3.1 main() 入口检查 planned-restart 标记（v3 补丁只改 alert 脚本、漏了 self_heal 脚本——主动重启的 OnFailure 副作用不再触发自愈循环，删除标记 return 0）②画像日更（c6362fa3e072）+ prep（5a2cdb97ffdd）prompt 加内存护栏（session_search limit≤20 分多轮、单轮读 ≤3 会话全文、>48h 窗口只检索最近 20 个、prep 分批每批 ≤5 会话）③hermes-gateway.service 加 MemoryHigh=10G / MemoryMax=12G（sed 追加 [Service] 段，daemon-reload 后 cgroup 实时生效无需重启）④$USER_profile_check.sh 改双判据——画像 mtime 旧但画像 job 输出目录最新文件新 = fresh（job 合法 [SILENT]），两者都 >48h 才 stale。补跑：cronjob run 画像日更/prep/鲜度检查/落盘验证全部 ok，8/5-prep-summary.json 已生成（10 会话 10 文件）。备份：crash_self_heal.py.bak.*、hermes-gateway.service.bak.*、jobs.json.bak.20260806073321。verify：11/11 ad-hoc 脚本 PASS（/tmp/hermes-verify-cron-oom-fixes.py）。

- **2026-08-06 provenance 失明修复（T79，内部多轮独立审查核实）**: write_file_hook + web-search-logger 从 config.yaml plugins.enabled 被 8/4 21:05 `hermes config set` 整表覆盖误丢 → gateway 重启后不再加载 → provenance 日志停更 36h+ → 8/6 09:15 健康检查 cron 报警。双故障：①基于 sed -n '784,800p' 截断视图（15 项）覆盖真实 17 项列表（801-802 行的两个插件被截掉）②config set 把 plugins.enabled 写成 str 类型致全部 user 插件短暂失活（21:31 修复类型但未加回）。修复：①`hermes plugins enable write_file_hook web-search-logger`（config 20 项，备份 config.yaml.bak.2026080610*）②provenance_health_check.sh v2 加插件加载归因——日志停更先 grep config enabled 列表，插件未启用报 🚨 根因+恢复命令，启用才报日志异常（备份 .bak.20260806100449）③gateway 重启（10:21:04，PID 3382694）验证 `provenance_hook v3.0 registered (tracking 14 tools)` + web-search-logger registered + 10:28 IM会话写入被记录。cron 重跑 last_status error→ok。复盘：审计/provenance失明_T79_事故复盘_v1_20260806.md。verify: `grep -c "provenance_hook v3.0 registered" ~/.hermes/logs/agent.log` ≥1 且 `tail -1 ~/.hermes/logs/write_file_hook.log` 为 2026-08-06。
