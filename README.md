...
- **假设列表**：[待标注]
- **建议审阅方式**：[待标注]
> **97.5% KV Cache 命中率。不是编程场景，是跨业务场景——架构设计、管理辅导、财务分析、零售渠道策略、招投标评审、PPT 生成、视频分析。月均 86 亿 input tokens，¥1,004/月。没有后端优化，纯上下文工程。**
这是一个生产级 AI 助手系统的架构佐证材料。含行为宪法、插件代码、运维脚本、方法论——证明系统设计和工程实现能力。
[English version](#hermes-$AGENT_NAME-evergreen-agent)
---
## TL;DR
...
不是代码补全刷出来的命中率。68,147 次调用覆盖了：IT 架构方案设计、资深技术专家入职策略、家庭财务分析、零售渠道汇报、行业投资 PPT 重做、视频深度分析、招投标评审——一个真实架构师的日常工作流。
...
---
## 为什么不是「开箱即用」
这个仓库来自一个运行两个月的生产系统。它曾经连接着外部基础设施、同步着多用户画像、推送着消息通知。那些能力涉及个人数据和特定基础设施，无法开源。
...
> **97.5% KV Cache hit rate. Not code completion. Cross-business scenarios—architecture design, management coaching, financial analysis, retail channel strategy, bid evaluation, PPT generation, video analysis. 8.6 billion input tokens/month, ¥140/month. No backend optimization. Pure context engineering.**
...
This isn't a coding benchmark. Those 68,147 calls spanned: IT architecture design, senior-engineer onboarding strategy, household financial analysis, retail channel presentations, industry investment deck rebuilds, deep video analysis, bid evaluation—the actual daily workflow of an enterprise architect.
...
This repo comes from a two-month production system. It once connected to external infrastructure, synced multi-user profiles, and pushed message notifications. Those capabilities involve personal data and specific infrastructure that can't be open-sourced.
...
- **[hermes-memory-architecture](https://github.com/ohtokaah-sys/hermes-memory-architecture)** — 记忆三层渐进式披露架构开源版：MEMORY 摘要分层注入 + 约束机械守护 + supersedes 去重 + 写入分流 + TTL 生命周期
- **[ppt-master-kit](https://github.com/ohtokaah-sys/ppt-master-kit)** — PPT 设计 DNA 工具链：三层 DNA 提取/映射/对比/重做 + 视觉逆向方法论

### 方法论沉淀

- [Open-Source Release Privacy Audit](https://github.com/ohtokaah-sys/knowledge-base/blob/master/articles/open-source-privacy-audit-methodology.md) — 开源发布隐私审计四件套：词表分层 / LLM 双模型语义审计 / git 历史盲区（孤儿分支重建）/ 案例全杀
